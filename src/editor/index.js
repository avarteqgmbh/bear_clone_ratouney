...
const RenderEditor = function RenderEditor({ current, selected, categories, onTitleUpdate, onBodyUpdate, onChangeCategory }) {
    return (
        <div>
                                    ...
                                    <Select defaultValue={current.category} onChange={(e) => onChangeCategory(selected, e)} >
                                        {
                                            categories.map((categ) => (
                                                <Select.Option key={categ} value={categ}>{categ}</Select.Option>
                                            ))}
                                    </Select>
                                </Col>
                            </Row>
...
function mapStateToProps(state) {
    const current = _.findIndex(state.NoteListReducer.notes, (note) => (note.id === Number(state.NoteListReducer.selected)));
    return {
