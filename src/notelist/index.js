...
const RenderNoteList = function RenderNoteList({ onClickNote, onAddNote, listName, notes, selected, onHoverNote, onUnhoverNote, hover, hovering, onRemoteAdd }) {
  return (
    <Menu
      theme="dark"
      mode="inline"
    ...
    >
      <Menu.SubMenu key={listName} title={<span className="nav-text" style={{ fontSize: '16px' }} >{listName}</span>} >
        ...
        <Menu.Item key={'remote_add_note'}><Icon type="plus-square" /><span style={{ color: 'purple' }}> REMOTE NEW NOTE</span></Menu.Item>
      </Menu.SubMenu>
    </Menu>
  );
};
